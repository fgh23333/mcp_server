...
class SimpleMetaToolAgent:
    ...
    def _tool_writer_node(self, state: SimpleMetaToolAgentState) -> SimpleMetaToolAgentState:
        ...
        if code_match:
            ...
            print("    INFO: -> 已成功从LLM响应中提取Python代码块。")
        else:
            print("    WARNING: -> 未在LLM响应中找到 ```python ... ``` 代码块，将尝试直接使用全部内容。")
    ...
    def _test_and_save_node(self, state: SimpleMetaToolAgentState) -> SimpleMetaToolAgentState:
        print(f"INFO: 节点: 2. 测试并保存代码 (重试次数: {state.retries-1})")
    ...
    def invoke(self, initial_state: SimpleMetaToolAgentState) -> SimpleMetaToolAgentState:
        print("INFO: 正在调用 LangGraph invoke 获取最终状态...")
        try:
            ...
            if final_state.messages:
                print(f"INFO: Invoke 完成。最终消息: {final_state.messages[-1].content[:200]}...")
            else:
                print("WARNING: Invoke 完成，但 final_state.messages 为空。")
...
def create_simple_mcp_tool(tool_description_request: str) -> str:
    """
    ...
    """
    print(f"INFO: 调用 'create_simple_mcp_tool'。请求: '{tool_description_request[:100]}...'")
    ...
    try:
        ...
        print(f"INFO: 'create_simple_mcp_tool' 完成。最终返回: {all_messages_content[:200]}...")
